...
class ByteArray {
    ...
    public byte get(int index) throws IndexOutOfBoundsException {
        if (index < 0 || mLength <= index) {
            // Bad index.
            throw new IndexOutOfBoundsException(String.format(Locale.US, "Bad index: index=%d, length=%d", index, mLength));
        }
    ...
    public byte[] toBytes(int beginIndex, int endIndex) {
        ...
        if (len < 0 || beginIndex < 0 || mLength < endIndex) {
            throw new IllegalArgumentException(
                    String.format(Locale.US,"Bad range: beginIndex=%d, endIndex=%d, length=%d",
